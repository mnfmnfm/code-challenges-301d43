...
function myMap(arr, fun) {
  const answer = [];
  // goes through every item of the array
  //   applies the fun
...
const mapTwoToThe = (input) => {
  return myMap(input, x => Math.pow(2, x));
...
const charCode = (input) => {
  return input.map(x => x.charCodeAt(0));
};
...
const evenOdd = (input) => {
  return input.map(x => (typeof(x) === 'number' ? (x % 2 === 0 ? 'even' : 'odd') : 'N/A'));
};
