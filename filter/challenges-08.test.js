...
const oddValues = (input) => input.filter(x => x % 2);
...
const filterStringsWithVowels = (input) => input.filter(x => /[aeiou]/.test(x));
...
const getBaseStatGreaterThan = (input, minBaseStat) => input.filter(x => x.baseStat > minBaseStat);
...
const getStatName = (input, minBaseStat) => input.filter(x => x.baseStat > minBaseStat).map(x => x.stat.name);
...
const getCharactersWithoutChildren = (input) => input.filter(person => !person.children);
