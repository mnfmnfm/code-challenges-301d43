...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  let answer = false;
  arr.forEach(houseData => {
    getFrom(houseData, 'entries').forEach(entry => {
      if(entry[0] === 'name' && entry[1] === character && houseData.children.length) {
        answer = true;
      }
    });
  });
...
const houseSize = (arr) => {
  ...
  getFrom(arr,'values').forEach((character) => {
    let houseCount = character.children.length + 1;
    if(character.spouse) houseCount++;
    let bundle = {house: character.house, members: houseCount};
    result.push(bundle);
  });
  return result;
}
...
const houseSurvivors = (arr) => {
  ...
  getFrom(arr, 'values').forEach((character) => {
    let houseCount = character.children.length + 1;
    if(character.spouse){
      if(deceasedSpouses.indexOf(character.spouse) < 0 ) houseCount++; 
    }
    let bundle = {house: character.house, members: houseCount};
    result.push(bundle);
  });
